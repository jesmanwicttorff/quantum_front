    ...
    <v-row>
        <v-col>       
            ...
            <v-tabs-items v-model="tab">
                ...
                <v-tab-item id="tab-4">
                    <IngresosSolicitudesDetalles v-if="this.$route.params.tipo == 13 || user.tipo == 13" :fechasConsulta="this.fechas()" />        
                </v-tab-item>                
            </v-tabs-items>            
            <ModalLoading/>
            <template>
...
export default {
    ...
    data(){
        return{
            ...
            bodyExcelDetalles: [],
            materia_id: 0,
            urlquauntum: url2 + '/indicadoresPenal/Ingresos/'+ store.get('token') + '/' + store.get('cod_corte') + '/' + store.get('cod_tribunal') + '/' + store.get('ano') + '/' + store.get('mes') + '/' + 0 + '/' + store.get('exhorto') + '/' + store.get('tipo')           
        }
    },
    beforeCreate(){
