...
export default {
  ...
  methods: {
    ...
    requestData: function (dateRange) {
      ...
      const req1 = url + '/familia/centrosCumplimientos'
      const req2 = url + '/familia/centrosCumplimientosDetalles'
      ...
      axios.all([
        ...
        Object.values(data2.recordset).map((type) => {
            this.items.push({isActive: true, gls_centroresidencial: type.gls_centroresidencial, 
                            ...
                            gls_tipolitigante: type.gls_tipolitigante, fecha_resolucion: type.fecha_resolucion, flg_80bis: type.flg_80bis, dia: type.dia.toString().replace(/\B(?=(\d{3})+(?!\d))/g, '.'), 
