      ...
      <b-container>
        <b-container class="tile_count">
          <b-row class="tile_stats_count">
            ...
            <b-col sm="6">
              <span class="count_top">Total otros Libros</span>
...
export default {
  ...
  data () {
    return {
      ...
      chartOptions: {
        ...
        title: {
          text: 'Porcentaje de ingreso por libros'
        },
        accessibility: {
          announceNewData: {
            enabled: true
        ...
        xAxis: {
          type: 'category',
          categories: ['Antofagasta', 'Arica', 'Chillan', 'Concepción', 'Copiapó', 'Coyhaique', 'Iquique', 'La Serena', 'Puerto Montt', 'Punta Arenas', 'Rancagua', 'San Miguel', 'Santiago', 'Talca', 'Temuco', 'Valdivia', 'Valparaíso'],
          labels: {
            rotation: -45,
            style: {
              fontSize: '10px',
  ...
  methods: {
    requestData: function (dateRange) {
      ...
      const ing_url = url + '/apelaciones/cortesIngresosLibrosTotales'
        ...
        try {
          ...
          Object.values(data.recordset).map((type) => {
            if (libro != '' && libro != type.libro) {
              console.log(type.libro)
              this.chartOptions.series.push({ name: libro, data: cantidad })
              cantidad = []
            }
            this.endVal += type.cantidad
            this.endVal2 += (type.libro == 'OTROS') ? type.cantidad : 0
            libro = type.libro
            cantidad.push(type.cantidad)
          })
          this.chartOptions.series.push({ name: libro, data: cantidad })
        } catch (error) {
          console.log(error)
